...
% Let $\vec{P}$ represents points lying on the YZ plane. Since \myvec{1\\0\\0} is the normal vector to the plane and the origin($\vec{O}$) lies on the YZ plane, vector equation of YZ plane is
% \begin{align}
%     &\myvec{1 & 0 & 0}(\vec{P}-\vec{O})=0\\
%     &\implies \myvec{1 & 0 & 0}\vec{P}=0 \label{eq-1}
% \end{align}
...
\begin{align}
    ...
    \vec{P}=\dfrac{\vec{A}+k\vec{B}}{k+1}
\end{align}
Vector equation of YZ plane with normal vector $n$ and perpendicular distance from origin $d$ is
\begin{align}
    n^\top\vec{X}=d
\end{align}
...
\begin{align}
    &n^\top\vec{P}=d\\
    \implies &n^\top\left(\dfrac{\vec{A}+k\vec{B}}{k+1}\right)=d\\
    \implies &n^\top\vec{A}+kn^\top\vec{B}=d(k+1)
\end{align}
For YZ plane, $n=\myvec{1\\0\\0}$ and $d=0$. So,
\begin{align}
    &\myvec{1 & 0 & 0}\vec{A}+k\myvec{1 & 0 & 0}\vec{B}=0\\
    \implies &\myvec{1 & 0 & 0}\myvec{4\\8\\10}+k\myvec{1 & 0 & 0}\myvec{6\\10\\-8}=0
\end{align}
\begin{align}
    \implies &4 + 6k = 0\\
    \implies &k = -2/3 
